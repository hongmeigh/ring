/**
 * @name jQuery.extend
 * @file 对jQuery做了些扩展，以下所有JS都依赖与此文件
 * @desc 对jQuery一些扩展，组件必须依赖
 ...
 * @import core/jQuery.js, core/jQuery.extend.js
 */
(function($, undefined) {
    var id = 1,
        _blankFn = function(){},
    ...
    $.ui = $.ui || {
         ...
         *
         * **Tips**
         * 1. 通过jQuery对象上的组件方法，可以直接实例话组件, 如: $('#btn').button({label: 'abc'});
         * 2. 通过jQuery对象上的组件方法，传入字符串this, 可以获得组件实例，如：var btn = $('#btn').button('this');
         * 3. 通过jQuery对象上的组件方法，可以直接调用组件方法，第一个参数用来指定方法名，之后的参数作为方法参数，如: $('#btn').button('setIcon', 'home');
    ...
    $.extend(_widget.prototype, {
         ...
         * </code>
         *
         * 所谓的setup方式，即，先有dom，然后通过选择器，初始化jQuery后，在jQuery对象直接调用组件名方法实例化组件，如
         * <code>
         * //<div id="widget"></div>
         ...
         * @desc 绑定事件，此事件绑定不同于jQuery上绑定事件，此On的this只想组件实例，而非jQuery实例
         */
...
// Prototype
    iScroll.prototype = {
        ...
        refresh: function () {
            ...
            that.wrapperW = document.body.clientWidth || 1;
 ...
 */
/**
 * @file GMU定制版iscroll，基于[iScroll 4.2.2](http://cubiq.org/iscroll-4), 去除zoom, pc兼容，snap, scrollbar等功能。同时把iscroll扩展到了jQuery的原型中。
 * @name jQuery.iScroll
 * @import core/jQuery.js
 * @desc GMU定制版iscroll，基于{@link[http://cubiq.org/iscroll-4] iScroll 4.2.2}, 去除zoom, pc兼容，snap, scrollbar等功能。同时把iscroll扩展到了***jQuery***的原型中。
 */

/**
 * @name iScroll
 * @grammar new iScroll(el,[options])  ⇒ self
 * @grammar $('selecotr').iScroll([options])  ⇒ jQuery实例
 * @desc 将iScroll加入到了***$.fn***中，方便用jQuery的方式调用iScroll。
 * **el**
...
(function ($, undefined) {
     ...
     * @desc
     * **Options**
     * - ''container''       {Selector|jQuery}:    (可选)父容器，渲染的元素，默认值：document.body
 ...
 * @import core/jQuery.iscroll.js, widget/navigator.js
 */
